...
public interface HostRepository extends JpaRepository<Host, Long> {
    ...
    @Query("select host from Host host left join fetch host.hamsters where host.hostStatus = 'ACTIVE' and host.address.town like concat('%',:city,'%')")
    ...
    @Query("select host from Host host left join fetch host.hamsters where host.name = :name")
    Host findByNameWithAllHamsters(@Param("name") String name);
